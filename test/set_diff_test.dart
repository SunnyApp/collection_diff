...
void main() {
  group("all", () {
    ...
    test("Model identity sanity checks", () async {
      final defaultEq = DiffEquality<Renamable>();
      expect(
          defaultEq.identical(
      ...
      expect(defaultEq.identical(rich, rich), equals(true),
          reason: "same identity");

      final set = EqualitySet<Renamable>(defaultEq.areIdentical);
      set.add(Renamable.ofId("1", "Richard"));

      expect(defaultEq.areIdentical.isValidKey(Renamable.ofId("1", "Richard")),
          equals(true),
    ...
    test("Set diff - update item with alternate equality", () async {
      ...
      final diff = set1.differences(set2,
          equality:
              DiffEquality(areIdentical: DefaultDiffEquality<Renamable>()));
